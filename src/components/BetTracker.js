...
const BetTracker = () => {
  ...
  // Define fetchBets as a memoized function to avoid recreation on every render
  ...
  // The fetchBets function is already defined above
  ...
  // Function to sync checked bets that aren't properly saved in Firebase
  const handleSyncCheckedBets = async () => {
    if (!userId) {
    ...
    try {
      console.log('[BetTracker] Starting bet synchronization');
      
      // 1. Get all bets that are currently saved in Firebase
      const savedBets = [...bets];
      console.log('[BetTracker] Current saved bets:', savedBets.length);
      
      // 2. Get all checked bets from localStorage (stored by EVsPage)
      const checkedBetsJson = localStorage.getItem('checkedBets');
      let checkedBets = [];
      
      if (checkedBetsJson) {
        try {
          checkedBets = JSON.parse(checkedBetsJson);
          console.log('[BetTracker] Found checked bets in localStorage:', checkedBets.length);
        } catch (error) {
          console.error('[BetTracker] Error parsing checked bets from localStorage:', error);
        }
      } else {
        console.log('[BetTracker] No checked bets found in localStorage');
      }
      
      // 3. Use the syncCheckedBets utility to sync any missing bets
      const result = await syncCheckedBets(checkedBets, savedBets, userId);
      console.log('[BetTracker] Sync result:', result);
      
      // 4. Update state with the result
      setSyncResult(result);
      setSyncSnackbarOpen(true);
      
      // 5. Refresh bets to show newly synced bets
      if (result.synced > 0) {
        fetchBets();
      }
    } catch (error) {
      console.error('[BetTracker] Error syncing checked bets:', error);
  ...
  // Set up listener for bet added events using our utility function
  useEffect(() => {
    console.log('[BetTracker] Setting up betAdded event listener');
    const removeListener = addBetAddedListener((betDetails) => {
      console.log('[BetTracker] Bet added event received via betSyncUtils with details:', betDetails);
      console.log('[BetTracker] Refreshing bets after betAdded event');
      fetchBets();
    });
  ...
  // Fetch bets when component mounts or when dependencies change
  useEffect(() => {
    console.log('[BetTracker] Initial fetch of bets');
    fetchBets();
  }, [fetchBets, selectedMonth, selectedYear, unitValue]);
  
  // Load initial unit value
  useEffect(() => {
    loadUnitValue();
  }, []);
  ...
  // Effect for storage event listener and interval check
  useEffect(() => {
  ...
  // Legacy event listener for betAdded (can be removed once betSyncUtils is fully implemented)
  useEffect(() => {
    const handleBetAdded = (event) => {
      console.log('[BetTracker] Detected legacy betAdded event with details:', event.detail);
      console.log('[BetTracker] Refreshing bets after legacy betAdded event');
      fetchBets();
    };
  ...
  return (
    <Box sx={{ p: 2 }}>
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} display="flex" justifyContent="flex-end" mb={2}>
            <Button
              variant="contained"
              color="primary"
              onClick={handleSyncCheckedBets}
              disabled={isSyncing}
              startIcon={isSyncing ? <CircularProgress size={20} /> : <SyncIcon />}
              sx={{ ml: 'auto' }}
            >
              {isSyncing ? 'Syncing...' : 'Sync Checked Bets'}
            </Button>
          </Grid>
          <Grid item xs={6} sm={2}>
      ...
      >
        ...
        >
          {syncResult?.success 
            ? `Sync complete: ${syncResult.synced} bet${syncResult.synced !== 1 ? 's' : ''} synced` 
