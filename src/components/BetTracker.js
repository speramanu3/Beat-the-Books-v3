...
const BetTracker = () => {
  ...
  const calculateStats = (betsData) => {
    ...
    // Calculate net profit in units - DO NOT use toFixed here as it returns a string
    const netProfitUnits = unitsWon - unitsLost;
    
    // Calculate total profit in dollars using unit value - use Math.round for proper decimal precision
    const totalProfit = Math.round(netProfitUnits * unitValue * 100) / 100;
    
    // Calculate ROI - use direct calculation
                    ...
                    <TableCell>
                      {(() => {
                        ...
                        if (lowerTrimmedTeam === 'over' || lowerTrimmedTeam === 'under') {
                          ...
                          // This implies 'trimmedTeam' was not 'over' or 'under'
                          if (typeof bet.line !== 'undefined' && bet.line !== null && String(bet.line).trim() !== '') {
                            return `${bet.homeTeam} vs ${bet.awayTeam} Total ${bet.line}`;
                          } else {
                            return `${bet.homeTeam} vs ${bet.awayTeam} Total (No Line)`;
                          }
                        }
                        // Fallback for any other case or missing data
                        return trimmedTeam || 'N/A';
                      })()}
                    </TableCell>
